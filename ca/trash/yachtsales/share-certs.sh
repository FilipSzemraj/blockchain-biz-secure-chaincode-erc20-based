...
mkdir /etc/hyperledger/_shared_certs/"$HOSTNAME"-msp/
mkdir /etc/hyperledger/_shared_certs/"$HOSTNAME"-msp/cacerts/
mkdir /etc/hyperledger/_shared_certs/"$HOSTNAME"-msp/tlscacerts/

cp "$TLS_CA_CERT" "/etc/hyperledger/_shared_certs/$HOSTNAME-msp/tlscacerts/tls-ca-cert.pem"
cp "$CA_CERT" "/etc/hyperledger/_shared_certs/$HOSTNAME-msp/cacerts/ca-cert.pem"

sleep 3

PATHS=$(find /etc/hyperledger/_shared_certs -type f -path "*/tlscacerts/*" ! -path "*$HOSTNAME*" | paste -sd ',' -)
