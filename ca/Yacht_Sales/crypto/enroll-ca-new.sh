...
export FABRIC_CA_CLIENT_CANAME=yachtsales-ca


fabric-ca-client enroll -u https://admin:adminpw@localhost:7056 \
...
mv /etc/hyperledger/client/ca/admin/msp/keystore/* /etc/hyperledger/client/ca/admin/msp/keystore/yachtsales-key.pem
...
for USER in "${USERS[@]}"; do
 # Rozdzielenie loginu i hasła
   ...
   CMD="fabric-ca-client enroll -u https://$USERNAME:$PASSWORD@localhost:7056 \
