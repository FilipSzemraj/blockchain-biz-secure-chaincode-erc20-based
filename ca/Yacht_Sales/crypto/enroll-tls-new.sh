...
fabric-ca-client enroll -d -u https://tlsadmin:tlsadminpw@localhost:7056 --tls.certfiles $TLS_CA_CERT --mspdir tls-ca/tlsadmin/msp --csr.hosts "tlsadmin,localhost"


#fabric-ca-client enroll -d -u https://admin:adminpw@localhost:7056 --tls.certfiles $TLS_CA_CERT --enrollment.profile tls --csr.hosts "yachtsales-ca,localhost,127.0.0.1,admin" --mspdir tls-ca/admin/msp
...
for USER in "${USERS[@]}"; do
  ...
  fabric-ca-client enroll --enrollment.profile tls \
    -u https://$USERNAME:$PASSWORD@localhost:7056 \
    --caname tls-ca \
...
cp -r /etc/hyperledger/client/tls-ca/yachtsales-ca/msp/signcerts/cert.pem /etc/hyperledger/server/ca/tls/cert.pem
cp -r /etc/hyperledger/client/tls-ca/yachtsales-ca/msp/keystore/* /etc/hyperledger/server/ca/tls/key.pem
